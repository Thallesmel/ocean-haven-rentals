...
export function ICSCalendarPreview() {
  ...
  const notedDates = useMemo(() => Object.keys(notes).map((d) => new Date(d)), [notes]);
  ...
  const blockSelectedRange = () => {
    if (!selected?.from || !selected?.to) return;
    setDisabledRanges((prev) => [...prev, { from: selected.from, to: selected.to }]);
  };
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    checked={(() => {
                      ...
                      const days = eachDayOfInterval({ start: selected.from, end: selected.to });
                      const vals = days.map((d) => availability[format(d, "yyyy-MM-dd")] ?? true);
                ...
                <div className="space-y-2">
                  ...
                  <Button
                    ...
                    disabled={!selected?.from || !selected?.to || !noteDraft.trim()}
                  >
